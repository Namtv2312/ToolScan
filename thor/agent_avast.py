...
def parser1(filepath):
    try:
        def get_previous_line(filepath, target_line):
            with open(filepath, 'r') as file:
                lines = file.readlines()

            for i in range(len(lines) - 1, 0, -1):
                if re.search(target_line, lines[i]):
                    return lines[i - 1].strip() if i - 1 >= 0 else None
            return None
        target_line = "# ----------------------------------------------------------------"
        result = get_previous_line(filepath, target_line)
...
def do_scan():
    def __kasper(file_path):
        ...
        cmd = f'ashCmd {file_path} /_> {log_path}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("port", type=int, nargs="?", default=8005)
