...
class ParsedYaraRules(yarabuilder.YaraBuilder):
    ...
    def parse_yara_rules(self, raw_rules):
        ...
        for match in rule_header_regex_matches:
            ...
            test = raw_rules[start_of_rule_content:]
            end_of_rule = (
                self._find_end_of_rule(raw_rules[start_of_rule_content:])
                #+ start_of_rule
                + start_of_rule_content
            )
            rule_ = raw_rules[start_of_rule:end_of_rule]
...
def main():  # pragma: no cover
    ...
    with open("D:/YARA/yaraml_rules-master/yaraml_generator/yaraml/powershell_model/powershell_detector1.yara", "r") as infile:
        raw_rule = infile.read()

    rules = ParsedYaraRules()
    rules.parse_yara_rules(raw_rule)
    tree0 = rules.yara_rules['tree0']
