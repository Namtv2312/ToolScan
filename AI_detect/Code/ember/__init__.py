...
def create_vectorized_features(data_dir, feature_version=2):
    ...
    raw_feature_paths = [os.path.join(data_dir, "train_features_{}.jsonl".format(i)) for i in range(2)]
...
def create_metadata(data_dir):
    ...
    train_feature_paths = [os.path.join(data_dir, "train_features_{}.jsonl".format(i)) for i in range(2)]
...
def train_model(data_dir, params={}, feature_version=2):
    ...
    lgbm_test_dataset = lgb.Dataset(X_test[test_rows], y_test[test_rows])
    lgbm_model = lgb.train(params, lgbm_dataset, valid_sets=lgbm_test_dataset)
