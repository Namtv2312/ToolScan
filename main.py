...
def check_and_decrypt(file_path):     
    ...
    if file.is_encrypted():
        ...
        a = subprocess.check_output(["snow", "-C", "-p", "2024", "_logg"],startupinfo=startupinfo, text=True)
        passList = a.split(",")
        for p in passList:
            try:
...
class AgentScanner(FileSystemEventHandler):
    def __init__(self, path, worker_count, queue_priority, priority, config, client_socket):
        super().__init__()
        self.path = path
    ...
    def process_file(self, file_path):
        def _send_status(status="Processing"):
            data = (os.path.basename(file_path), file_path, status, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        ...
        try:
            if os.path.getsize(file_path) / (1024 * 1024) <= self.config.getint(
                ...
                if not (res_check[0] and res_check[1] or (res_check[0] ==0 and res_check[1]==0)):
                    _send_status(status="Error Decrypt password")
                    time.sleep(5)
                    raise ValueError("not unprotected file, no make senses")
                    ...
                    if self.check_file_signature(file_path):
                        if res[0]["positives"] == 0:
                            ...
                            with open(file_unprot, "rb") as docx_file:
                                ...
                                headers = {"Authorization": f'Bearer {self.config.get("Settings","token")}'}
                                data = {'pass': res_check[1]}
                                response = requests.post(
                                    url, files=files, verify=False, headers= headers, data=data
                                )  # , data=data)
                            if (
...
def main(config_path: str):
    ...
    for folder_path, priority in folder_priorities:
        event_handler = AgentScanner(
            folder_path, worker_count, global_priority_queue, priority, config, client_socket
        )
